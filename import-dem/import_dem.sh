...
echo ""
first=1
for dem_file in `ls "$DEM_DIR"/*.hgt | sort -V`; do
...
raster2pgsql -c -I -Y -t 256x256 $HILLSHADE_DIR/*.hgt hillshade | PGPASSWORD=$POSTGRES_PASSWORD psql -U $POSTGRES_USER -d $POSTGRES_DB -h $POSTGRES_HOST -p $POSTGRES_PORT
