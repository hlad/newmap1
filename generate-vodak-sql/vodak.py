...
connection = connect("dbname='{}' user='{}' password='{}' port='{}' host='{}'".format(
	...
	os.environ['POSTGRES_PORT'],os.environ['POSTGRES_HOST']
))
...
for end,outfall in outfalls.items():
	if len(outfall) > 1:
		for osmId,name in outfall:
			...
			cur.execute(
				'''			
					SELECT ST_StartPoint(way) FROM osm_waterway WHERE osm_id = %d
				''' % osmId
			)		
		...
		for (end,(outfallId,outfallName)) in outfallsD[springName]:
			...
			cur.execute(
				'''
					SELECT
						ST_Distance(
							(SELECT ST_StartPoint(way) FROM osm_waterway WHERE osm_id = %d LIMIT 1),
							(SELECT ST_EndPoint(way) FROM osm_waterway WHERE osm_id = %d LIMIT 1)
						)			
				''' % (springId,outfallId)
			)
